...
void mainLoader(int, char*[], ServiceManager* services)
{
	...
	std::cout << "Compiled by Lord1Egypt" << std::endl;
	...
	if (!DatabaseManager::isDatabaseSetup()) {
		std::cout << ">> Empty database detected, importing schema_sqlite.sql..." << std::endl;
		std::ifstream schemaFile("schema_sqlite.sql");
		if (!schemaFile.is_open()) {
			startupErrorMessage("Could not open schema_sqlite.sql to initialize the database.");
			return;
		}
		...
		if (!db->executeQuery(schemaBuffer.str())) {
			startupErrorMessage("Failed to import schema_sqlite.sql into the database.");
			return;
		}
		std::cout << ">> Database schema imported successfully." << std::endl;
	}
